...
import {Button, Label, List, Segment, Item as SemanticItem, Container, Divider, Input} from "semantic-ui-react";
...
const useGetItems = () => {
    ...
    useEffect(() => {
        axios.get("https://ancient-coast-58289.herokuapp.com/api/items")
...
const onQuantityChange = (value: string, setTotal: Dispatch<SetStateAction<number>>, item: Item) => {
    if (isNaN(parseInt(value))) value = "0";
    itemQuanties[item.id] = parseInt(value);
    itemValues[item.id] = parseInt(value) * item.price;
    const sum = Object.values(itemValues).reduce((a, b) => (a as number) + (b as number), 0);
...
const processOrder = (total: number, items: Item[], history: any) => {
    ...
    Object.keys(itemQuanties).forEach(key => {
        if (itemQuanties[key] === 0 || itemQuanties[key] < 0) {
            ...
            const mainItem: OrderItem = {item: itemsObject[key], quantity: itemQuanties[key]};
        ...
        <>
            ...
            <Container>
                                ...
                                <SemanticItem.Content>
                                    <Segment floated={"right"} basic style={{padding: 0, margin: 0}}>
                                        <Input
                                            ...
                                            min={0}
                                            placeholder={0}
                                            onChange={((event, data) => onQuantityChange(data.value, setTotal, item))}
                                        />
                                    </Segment>

                                    <SemanticItem.Header>{`${item.name}`}</SemanticItem.Header>
                                    <SemanticItem.Meta
                                        style={{color: "red"}}>${item.price.toFixed(2)}</SemanticItem.Meta>
                                </SemanticItem.Content>
                            </List.Item>)
                    })}
                </SemanticItem.Group>
                ...
                <Container textAlign={"right"} style={{color: "red"}}>
                    ...
                    <Button style={{marginTop: 30}} color={"green"} onClick={() => processOrder(total, items, history)}>
                        Place Order
                    </Button>
                </Container>
            </Container>
