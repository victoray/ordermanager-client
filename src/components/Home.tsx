...
export const handleClick = (index: number, activeIndex: number, setIndex: Dispatch<SetStateAction<number>>) => {
...
const Home = () => {
    ...
    if (orders === undefined) return <Dimmer active><Loader/></Dimmer>;
    return (
        <>
            <NavBar/>
                    ...
                    {orders.sort((a, b) => a.id < b.id ? 1 : -1).map(order => {
                        return (
                            <React.Fragment key={order.id}>
                                <Accordion.Title active={activeIndex === order.id} index={order.id}
                                ...
                                <Accordion.Content as={Segment} active={activeIndex === order.id}
                                                   attached>
                                    {order.orderItems.map((orderitem, index) => {
                                        return (
                                            <Message info floating key={index}>
                                                <Message.Header>{orderitem.quantity} {orderitem.item.name}{(orderitem.quantity > 1) ? "s" : ""}</Message.Header>
                                                <p>${(orderitem.quantity * orderitem.item.price).toFixed(2)}</p>
                                            </Message>
                                        )
                                    })}
